...
const SharpPaymentsTableRow: FC<Props> = ({ data }) => {
  const { setModalContent } = useModalContext();

  return (
    <div className="flex p-3 py-6 items-center gap-6 border-b border-b[#E6E6E6]" onClick={() => {
      setModalContent(
      ...
      <div style={{ flex: 1 }} className="flex items-center">
        <p className="text-xs font-bold">{data?.amountRemaining}</p>
      </div>
...
export default SharpPaymentsTableRow;
