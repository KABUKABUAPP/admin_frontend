...
const TransactionsTableRow: FC<Props> = ({
  ...
  return (
    <div className="flex p-3 gap-6 border-b border-b[#E6E6E6]">
      ...
      <div style={{ flex: 2 }}>
        {data?.destination && <OriginDestinationCell origin={data.origin} destination={data?.destination} />}
      </div>

      <div style={{ flex: 1 }}>
        {data?.riderName && data.driverName && <RiderDriverCell rider={data?.riderName} driver={data?.driverName} />}
      </div>
