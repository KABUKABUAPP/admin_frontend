    ...
    <Card elevation={true} maxWidth="400px" maxHeight="95vh">
      <div className="p-4 overflow-x-hidden relative" ref={ref}>
        ...
        <div className="flex p-4 justify-between">
          <div className="w-[25%]">
            {(driver.profile_image || driver.full_name) && (
              <Avatar
                imageUrl={driver.profile_image}
                fallBack={`${driver.full_name && driver.full_name[0]}`}
                size="md"
              />
            )}
          </div>
          <div className="w-[75%]">
            <p>{capitalizeAllFirstLetters(driver.full_name)}</p>
            <p>{capitalizeAllFirstLetters(driver.online_status)}</p>
          </div>
        </div>
...
const MapOverlay: React.FC<MapOverlayProps> = ({ onlineStatusDriver, onlineStatusRider, enableRiderOption, enableDriverOption }) => {
  ...
  useEffect(() => {
      if (data) {
        if (onlineStatusDriver === 'online') setDrivers(data?.online_drivers)
        if (onlineStatusDriver === 'offline') setDrivers(data?.offline_drivers)
        if (onlineStatusRider === 'online') setRiders(data?.online_riders)
        if (onlineStatusRider === 'offline') setRiders(data?.offline_riders)
      }
  }, [data, onlineStatusDriver, onlineStatusRider])
  ...
  useEffect(() => {
    if (drivers && riders) {
      const driversCoordinates = enableDriverOption ? drivers?.data?.map((d: any) => {
        ...
        ? d.coordinate[0] : parseFloat(d.coordinate[0]), personnel: d, type: 'driver', _id: d._id}
      }) : [];
      ...
      const ridersCoordinates = enableRiderOption ? riders?.data?.map((d: any) => {
        ...
        if (d.coordinate && d.coordinate.length > 0) return {lat: typeof d.coordinate[1] === 'number'
        ? d.coordinate[1] : parseFloat(d.coordinate[1]), lng: typeof d.coordinate[0] === 'number'
        ? d.coordinate[0] : parseFloat(d.coordinate[0]), personnel: d, type: 'rider', _id: d._id}
      }) : [];
